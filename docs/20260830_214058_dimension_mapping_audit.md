# Canonical dimension mapping audit — 2026-08-30 21:40 JST
...
- rule SHA-256: `f36aa69788d9c5356f41d0eb57648b770727a80c42e8c38a75a03e51207f21ed`
- generated mapping: `data/processed/_mappings/20260830_214058_dimension_mapping/`
...
fuzzy match、推計、按分は使用しない。raw source label、row kind、region、subcategory、geography type、parent region、geography semanticsをmapping rowに保持する。authored ruleはreview済みの`S08`／`S09`へscopeし、未reviewの新source IDでは既存ruleをsilentに流用せず停止する。
...
- GREEN: mapping unit/integration/CLI 16 test成功。
- full suite: 63 test成功、skip 0、branch計測total coverage 85.71%。
