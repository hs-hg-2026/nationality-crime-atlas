...
M5 historical series / drift / scheduled updates ← YOU ARE HERE
  ...
  ├─ clearance-share trend + arithmetic residual + schema-v7 dashboard view ✓
  └─ 2020–2024 regional / nationality / offense panel ← NEXT
```
...
official HTTPS source
  ...
  → data/processed/_catalog/artifacts.{jsonl,csv}
       ...
       └─ nca-build-clearance-share-trend                  2015–2024 national trend
              ↓
         nca-build-compact-export                           schema v7
...
6. The composition section shows all 26 categories across six offense groups as heatmap and 100% stacked views, with cleared-person/case and source/cluster controls.
7. Small denominator / sparse numerator warnings do not suppress rows. Zero-total composition rows show `構成比算出不能`, not a false 0%.
8. Values are descriptive public-data-derived reference ratios, clearance shares, or mechanical source differences, not official crime rates, causal claims, danger rankings, or statements about individual risk.
...
- [x] compact-export schema v7 and checked-in publication hash
