...
- Production run: `data/processed/_indicators/20260831_084358_indicators/`
...
source IDとartifactの対応表はrun `summary.json`の`source_artifacts`に埋め込んだ。publisher、official landing/download URL、source period/table、raw/processed path、retrieval time、revision、SHA-256を含む。
...
2. catalog inputは`processing_status=validated`に限定し、同一`source_id`の重複を停止する。
...
5. numerator cellの重複、negative count、missing mappingを停止する。
...
| Artifact | SHA-256 |
|---|---|
| `summary.json` | `8b0aad77fb25e749fe83a776ca26eda674ba845c8a0c7614fa8dbc25f87f64ac` |
...
`20260831_084201_indicators`は新field追加後にoutput schema versionを旧`1`のまま生成したpre-fix runであり、latestではない。schema versionを`2`へ修正した本runがcurrent canonical outputである。旧runはtimestamp付きの再生成可能artifactとして残すが、公開対象にしない。

## Test verification

- Indicator-focused: 10 test passed。
- Full suite: 73 test passed、skip 0。
- Branch計測を含むtotal coverage: 84.40%（required 80%をpass）。
- 新規failure tests: invalid period、population-scope drift、duplicate numerator cell、missing prefecture、incomplete exact mapping、duplicate catalog source。
